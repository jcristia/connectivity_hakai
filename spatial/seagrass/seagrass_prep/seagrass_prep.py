...
hakaisurveyed = r'C:\Users\jcristia\Documents\GIS\MSc_Projects\1Data_BASE\Seagrass\Seagrass_polys_Hakai_mapped\seagrass_polys_hakai_mapped.gdb\seagrass_2014'
...
# merge
sg_merge = arcpy.Merge_management([islandstrust_buff, bcmca, misc, ross, tsawwassen, hakaiCEC, hakaisurveyed, washington], "seagrass_all_01MERGE")
...
# make a copy first so that I will still have a record of the last near analysis
sg_final = arcpy.FeatureClassToFeatureClass_conversion(sg_delmed2, arcpy.env.workspace, 'seagrass_all_17FINAL')
...
sgb_buff = arcpy.Buffer_analysis('seagrass_all_17FINAL', 'seagrass_buff_01BUFF', 100, dissolve_option = 'NONE')
...
# merge
sgb_merge = arcpy.Merge_management(['seagrass_all_17FINAL', sgb_erase], 'seagrass_buff_04MERGE')
...
out_folder = r'C:\Users\jcristia\Documents\GIS\MSc_Projects\Hakai\spatial\seagrass\seagrass_all'
...
arcpy.DeleteField_management(os.path.join(out_folder, 'seagrass_all_17FINAL.shp'), ['Shape_Area', 'Shape_Leng'])
...
sg_all = r'C:\Users\jcristia\Documents\GIS\MSc_Projects\Hakai\spatial\seagrass\seagrass_all\seagrass.gdb\seagrass_all_17FINAL'
...
for fc in fcs:
    arcpy.AddField_management(fc, 'particle', 'LONG')
    ...
    arcpy.CalculateField_management(fc, 'particle', particle)
...
particle_check = 0
for fc in fcs:
    with arcpy.da.SearchCursor(fc, 'particle') as cursor:
        for row in cursor:
