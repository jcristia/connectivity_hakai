...
conn_fc = r'connectivity_metrics_connections_1'
...
arcpy.TableToTable_conversion(node_importances, arcpy.env.workspace, "node_importances")

nodes_fc = 'connectivity_metrics_patches'

# join
arcpy.MakeFeatureLayer_management (nodes_fc, 'nodes_fc')
arcpy.AddJoin_management('nodes_fc', "uID", 'node_importances', "Node")
arcpy.CopyFeatures_management('nodes_fc', 'connectivity_metrics_patches_conefor')

#arcpy.Delete_management('node_importances')
arcpy.DeleteField_management('connectivity_metrics_patches_conefor', drop_field=['node_importances_OBJECTID','node_importances_Node', 'node_importances_Field12'])
